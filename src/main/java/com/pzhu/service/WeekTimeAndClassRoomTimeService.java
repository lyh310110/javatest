...
//
//			return flag1;
		return courseMapper.setCourseStatusPassed(cid);
	}
	...
	public boolean setClassStatusPassed(int cid, int uid, int classsort, String[] startWeek, String[] endWeek, String[] classRoomTime) {
		...
		for (int i = 0; i < classRoomTime.length; i++) {
			...
			String[] str = classRoomTime[i].split("_");
			crtid = weekTimeAndClassRoomTimeMapper.getCrtid(str[0], str[1]);
			classRoomTimeMapper.setAlreadyStatus(crtid);
			weekTimeAndClassRoomTimeMapper.insertWeekTime(start, end, crtid);
			int wid = weekTimeAndClassRoomTimeMapper.getWid(start, end, crtid);
			// 改为更新而不是插入
			courseTeacherMapper.updateCorTherWithClassInfo(str[0], str[1], classsort, cid, uid, wid);
		}
		return true;
	}
}